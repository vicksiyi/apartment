...
CREATE TABLE `maintenance_type`  (
  ...
  `title` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '维修类别',
  `status` int(0) NOT NULL DEFAULT 1 COMMENT '0=>已删除 1 存在',
...
CREATE TABLE `maintenances`  (
  ...
  `type_id` int(0) NOT NULL COMMENT '维修类型',
  `room_rel_id` int(0) NOT NULL COMMENT 'user_rel_room id',
  `isEmergency` int(0) NOT NULL DEFAULT 0 COMMENT '0=>否 1=>是 是否紧急',
  `status` int(0) NULL DEFAULT 0 COMMENT '维修状态 0待维修 1待处理 2已完成',
  ...
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `maintenances_room_rel_id`(`room_rel_id`) USING BTREE,
  INDEX `maintenances_type_id`(`type_id`) USING BTREE,
  CONSTRAINT `maintenances_room_rel_id` FOREIGN KEY (`room_rel_id`) REFERENCES `user_rel_room` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT `maintenances_type_id` FOREIGN KEY (`type_id`) REFERENCES `maintenance_type` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci ROW_FORMAT = Dynamic;
...
) ENGINE = InnoDB AUTO_INCREMENT = 164 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '房屋信息与房屋类型信息关联表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `room_type`  (
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 18 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '房屋类型表' ROW_FORMAT = Dynamic;
...
-- ----------------------------
-- Triggers structure for table maintenances
-- ----------------------------
DROP TRIGGER IF EXISTS `maintenances_trigger`;
delimiter ;;
CREATE TRIGGER `maintenances_trigger` BEFORE INSERT ON `maintenances` FOR EACH ROW BEGIN
			set new.time = NOW();
    END
;;
delimiter ;
